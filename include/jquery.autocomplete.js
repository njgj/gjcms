...
$.autocompleter=function(myform,obj,action,width){
	  $(obj).autocomplete("ajaxJson.asp?o="+action, {
		max: 12,    //列表里的条目数
		...
		extraParams: {v:function(){return escape($(obj).val());}},
		cacheLength:0,
		//需要把data转换成json数据格式
		parse: function(data) {
			return $.map(eval('data.'+action), function(row) {
				return {
					data: row,
					value: row.name,
					result: row.name
		...
		formatItem: function(row, i, max) {
			...
			return  row.name + '<span>' + row.sm + '</span>';
		},
		formatMatch: function(row, i, max) {
			return row.name + row.sm;
		},
