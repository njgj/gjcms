...
switch($_GET['Action']){
    case 'order':
		...
		$sql = "update g_nclass_select set orderid=$_POST[orderid] where classid=$_GET[classid]";
		$db -> query($sql);
...
<thead>
    ...
    <tr>
      ...
      <tr>
        ...
        <option value="0">-----顶级分类-----</option>
            <?php
			    $class_arr=getClassArray('g_nclass_select');
  ...
  <?php
	...
	case 'act_add':
		...
		$sql = "INSERT INTO g_nclass_select (classname,parentid,orderid) VALUES('".$_POST['classname']."',".$parentid.",".$_POST['orderid'].")";
		$db -> query($sql);
		...
		break;
	case 'edit':
	$sql  = "select * from g_nclass_select where classid=".$_GET['classid'];
	$query = $db -> query($sql);
	...
	case 'act_edit':
		...
		$sql  = "select classid from g_nclass_select where classid=".$_POST['classid'];
		$query = $db -> query($sql);
		$row = $db -> fetch_array($query);
		if($row){
			if($row['classid']==$_POST['parentid']){
				...
				$sql = "update g_nclass_select set classname='".$_POST['classname']."',parentid=".$_POST['parentid'].",orderid=".$_POST['orderid']." where classid=".$_POST['classid'];
				$db -> query($sql);
	...
	case '':
		$class_arr=array();
		$sql = "select * from g_nclass_select order by orderid";
		$query = $db -> query($sql);
    ...
    <tr>
      ...
      <td height="25" align="center" background="../images/bg.gif" >分类名称</td>
...
//$class_arr[] = array($row['classid'],$row['classname'],$row['parentid'],$row['orderid']);
//$m:填充长度 $fid:父类ID
function nclass_select_arr($m=0,$fid=0)
{
	global $class_arr;
	if($fid=="") $fid=0;
	...
	for($i=0;$i<count($class_arr);$i++){
		...
		echo "</td>\n";
		echo "</tr>\n";		
			nclass_select_arr($m+1,$class_arr[$i][0]);
		}		
	}	
}
...
function delclass($classid){
	global $db;
    if($classid){
		//$db->query("delete from g_news where classid in (select classid from g_nclass_select where locate(',$classid,',classpath)>0)");
		$db->query("delete from g_nclass_select where locate(',$classid,',classpath)>0");
		return true;
	}else{
		return false;
	}
...
function savepath($newid,$parentid){
	...
	if($parentid==0){ 
		...
		$sql='select classpath from g_nclass_select where classid='.(int)$parentid;
		$query = $db -> query($sql);
		if($row = $db -> fetch_array($query)){
			$classpath=$row['classpath'].$newid.','; //父节点的路径	
		}
	}
	return $db -> query("update g_nclass_select set classpath='$classpath' where classid=".(int)$newid);
}
?>