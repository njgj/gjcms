...
switch($_GET['Action']){
    case 'order':
		...
		$sql = "update g_nclass_city set orderid=$_POST[orderid] where classid=$_GET[classid]";
		$db -> query($sql);
...
<thead>
  ...
  <?php
	...
	case 'act_add':
		...
		$sql = "INSERT INTO g_nclass_city (classname,parentid,orderid) VALUES('".$_POST['classname']."',".$parentid.",".$_POST['orderid'].")";
		$db -> query($sql);
		...
		break;
	case 'edit':
	$sql  = "select * from g_nclass_city where classid=".$_GET['classid'];
	$query = $db -> query($sql);
	...
	case 'act_edit':
		...
		$sql  = "select classid from g_nclass_city where classid=".$_POST['classid'];
		$query = $db -> query($sql);
		$row = $db -> fetch_array($query);
		if($row){
			if($row['classid']==$_POST['parentid']){
				...
				$sql = "update g_nclass_city set classname='".$_POST['classname']."',parentid=".$_POST['parentid'].",orderid=".$_POST['orderid']." where classid=".$_POST['classid'];
				$db -> query($sql);
	...
	case '':
		$class_arr=array();
		$sql = "select * from g_nclass_city order by orderid";
		$query = $db -> query($sql);
...
function delclass($classid){
	global $db;
    if($classid){
		//$db->query("delete from g_news where classid in (select classid from g_nclass_city where locate(',$classid,',classpath)>0)");
		$db->query("delete from g_nclass_city where locate(',$classid,',classpath)>0");
		return true;
	}else{
		return false;
	}
...
function savepath($newid,$parentid){
	...
	if($parentid==0){ 
		...
		$sql='select classpath from g_nclass_city where classid='.(int)$parentid;
		$query = $db -> query($sql);
		if($row = $db -> fetch_array($query)){
			$classpath=$row['classpath'].$newid.','; //父节点的路径	
		}
	}
	return $db -> query("update g_nclass_city set classpath='$classpath' where classid=".(int)$newid);
}
?>